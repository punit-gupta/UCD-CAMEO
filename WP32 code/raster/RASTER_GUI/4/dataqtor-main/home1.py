...
if uploaded_file is True:
    ...
    **Get your data ready for use before you start working with it:**
    ...
    if (type=="Landsat-8 Bands"):
        data = st.selectbox("Data", ["IMG_DATA2/*B?*.tif", "IMG_DATA3/*B?*.tif", "Data 3"])
        arr_st = np.stack(read_land(data))
        ...
        if (task == "Spatial Data Quality"):
            ...
            with tab1:
                ...
                #print(len(v1))
                st.write("Percentage of No Data: "+str(100*(np.count_nonzero(np.isnan(v1))/(len(v1)*len(v1)))))
